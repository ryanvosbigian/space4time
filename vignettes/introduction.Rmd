...
This returns a list. The key pieces of data of interest are

The capture history data:

```{r echo= FALSE}
knitr::kable(head(sim.dat$ch_df))
```
...
```{r echo= FALSE}
knitr::kable(head(sim.dat$aux_age_df))
```
...
```{r echo= FALSE}
knitr::kable(sim.dat$sites_config)
```

`holdover_config`:

```{r echo= FALSE}
knitr::kable(sim.dat$holdover_config)
```

`sites_names`:

```{r echo= FALSE}
knitr::kable(sim.dat$sites_names)
```
