...
functions {
  ...
  matrix predict_ordered_logistic(vector alk_par_delta, vector alk_par_alpha, matrix mod_mat, int max_a_overall) {
    // print("called: predict_ordered_logistic");
    int num_alphas = num_elements(alk_par_alpha);

    // matrix[rows(mod_mat),num_alphas + 1] pred_mat;
    ...
    for (i in 1:rows(mod_mat)) {
      mu[i] = sum(mod_mat[i,1:cols(mod_mat)] .* alk_par_delta');
    }

    // old version
    // vector[rows(mod_mat)] mu = mod_mat * alk_par_delta;



    pred_mat[1:rows(mod_mat),1] = 1 - inv_logit(mu - alk_par_alpha[1]);
    for (k in 2:(num_alphas)) {
      pred_mat[1:rows(mod_mat),k] = inv_logit(mu - alk_par_alpha[k - 1]) - inv_logit(mu - alk_par_alpha[k]);
    }
    // old
    pred_mat[1:rows(mod_mat),num_alphas+1] = inv_logit(mu - alk_par_alpha[num_alphas]);

    // new ... would like to go back to the old version because it is vectorized but it seems to be having issues
    // for (i in 1:rows(mod_mat)) {
    //   pred_mat[i,num_alphas] = 1 - sum(pred_mat[i,1:(num_alphas-1)]);
    // }

    // maybe this will cause problems
    if (max_a_overall > num_alphas + 1) {
      for (i in (num_alphas + 2):max_a_overall) {
...
model {
 ...
 //  // print("pi_M: ",pi_M[1:10,]);
  ...
  for (i in unknownage_l) {
    ...
    vector[tmp_a_end - tmp_a_start + 1] pi_age1 = pi_L[i,(tmp_a_start):(tmp_a_end)]'; // insures that pi_age1 will not be greater than max_a
 ...
 for (i in unknownage_m) {
    ...
    // determine pi_age
     int tmp_a_end = min({max_a_overall - diff_age, max_a_overall}); // insures that pi_age1 will be at max lgth max_a
    int tmp_a_start = 1 + abs(min({0,diff_age})); // returns 1 if diff age is greater than 0. if diff_age is negative,
    vector[tmp_a_end - tmp_a_start + 1] pi_age1 = pi_M[i,(tmp_a_start):(tmp_a_end)]'; // insures that pi_age1 will not be greater than max_a
...
generated quantities {
  ...
  // log_lik[1:N_obsageclass] = ordered_logistic_lpmf(obsageclass | mod_mat_a_beta * alk_par_delta,alk_par_alpha);
  for (i in 1:N_obsageclass) {
    log_lik[i] = ordered_logistic_lpmf(obsageclass[i] | mod_mat_a_beta[i,] * alk_par_delta,alk_par_alpha);
  }
