...
//                                                                                        max_a_overall);
  ...
  for (i in knownage_l) {
    ...
    int k_age = diff_age + obsageclass_L[i]; //obsageclass_L[i]
     ...
     log_lik[i] = log(chi_array[l_matrix[i,4],l_matrix[i,1],l_matrix[i,3],l_matrix[i,2],k_age]) * l_matrix[i,6];
  ...
  for (i in unknownage_l) {
    ...
    log_lik[i] = log(lik2) * l_matrix[i,6];
  ...
  for (i in knownage_m) {
    ...
    log_lik[i + N_l] = log(lik1) * m_matrix[i,8];
 ...
 for (i in unknownage_m) {
    ...
    log_lik[i + N_l] = log(lik2) * m_matrix[i,8];
